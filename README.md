...
5. **HTTP Status** — **17 códigos** con descripción, causas comunes, troubleshooting y consejos de seguridad.
...
└── vault/                  # ★ La app completa ★
    ...
    │   ├── GlossaryView.tsx
    │   ├── ReferencesView.tsx
    │   ├── ToolsView.tsx           # 8 herramientas en tabs
    ...
    │   ├── httpStatusData.ts    # 17 códigos
    │   ├── portsData.ts         # 79 puertos
    │   ├── winEventsData.ts     # 56 Event IDs
    │   └── cronData.ts          # Ejemplos de cron
    ├── db/index.ts          # Dexie schema (v10) — notas, labs, glosario, referencias, blobs, pdfs
...
# Prisma (opcional — el vault usa Dexie, pero el schema existe para features futuras)
bun run db:push
bun run db:generate
```
