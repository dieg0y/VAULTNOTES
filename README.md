...
- **Online opcional y explícito**:
  - *Sync de datasets oficiales (V10)* — ver sección **🌐 V10** abajo: MITRE ATT&CK, reglas Sigma oficiales y catálogo CISA KEV se descargan SOLO al pulsar **[Sync]** en Data & Intel. Sin API keys, sin cuentas, datos oficiales que quedan guardados offline.
  ...
  - *CVE Search* — consulta a NVD al buscar; offline muestra los CVEs guardados. Tras cada búsqueda muestra el badge **CISA KEV** (catálogo local) y el score **EPSS** (FIRST, cacheado 24 h).
...
## 🌐 V10 — Datasets oficiales actualizados (sync opcional, 0 USD)

> Informe de diseño: `docs/V10-ONLINE-SEARCH-ANALISIS.md`.

VaultNotes sigue siendo **local-first y offline-first**: el contenido bundle
(curado en español) es la base que SIEMPRE funciona sin internet. La capa V10
añade **información oficial actualizada** descargada con un clic explícito y
guardada para siempre en tu navegador:

| Dataset | Fuente oficial | Qué aporta | Peso aprox. |
|---|---|---|---|
| **MITRE ATT&CK** | `mitre-attack/attack-stix-data` (STIX 2.1, GitHub de MITRE) | ~700 técnicas oficiales vigentes (v19.x) fusionadas sobre las 61 curadas — nombres, descripciones, plataformas y sub-técnicas al día; las notas de detección en español del bundle se conservan | ~54 MB (streaming, memoria plana) |
| **Sigma oficial** | `SigmaHQ/sigma` → `rules/windows/builtin/security` (vía CDN jsDelivr que sirve el repo byte a byte) | 143+ reglas oficiales de detección añadidas al Sigma Explorer, dedupeadas por UUID contra las curadas | ~1 MB |
| **CISA KEV** | `cisagov/kev-data` (espejo GitHub oficial de cisa.gov) | 1.700+ CVEs con explotación conocida: el CVE Search muestra badge rojo **KNOWN EXPLOITED**, fechas, uso en campañas ransomware y la acción requerida oficial — **offline** tras sincronizar | ~1.7 MB (se guardan ~600 KB) |
| **EPSS** | `api.first.org` (FIRST, gratuito) | Score de probabilidad de explotación a 30 días junto a cada CVE buscado; cacheado 24 h en `tiCache` (viaja en el backup) | ~0 |

**Reglas que nunca cambian** (invariantes I1–I7 del análisis V10):

1. Nada se descarga automáticamente — solo al pulsar **[Sync]** / **[Search Online]**.
2. El bundle curado de la app SIEMPRE prevalece como base; lo sincronizado es una capa fusionada encima (dedupe por id/UUID).
3. Sin API keys, sin cuentas, sin PII en las consultas (solo URLs públicas y el CVE que ya estás buscando).
4. Offline: los botones se deshabilitan y todo lo demás funciona igual; lo ya sincronizado queda disponible para siempre.
5. Los datasets sincronizados viajan en el **backup ZIP 3.10.0** (`mitreTechniques.json`, `sigmaRulesSynced.json`, `kevCatalog.json`, merge latest-wins) — las claves API siguen JAMÁS en el ZIP.
6. El swap es atómico: si una descarga falla a mitad, el dataset anterior queda intacto.

Dónde está todo: **Data & Intel → Sync Center** (tarjetas MITRE / Sigma / CISA KEV con versión, contador, *Check for Updates* real contra la fuente y *Sync* con progreso). El MITRE Explorer y el Sigma Explorer consumen la capa fusionada automáticamente; el CVE Search muestra KEV+EPSS tras cada búsqueda.

---

## ✅ Verificación (estado actual)

> **v10 — Online dataset sync OFICIAL en el Sync Center (pasada 13, navegador real)**: los stubs honestos de MITRE/Sigma quedan cableados a fuentes reales y verificadas, + CISA KEV + EPSS. **Dexie v24 (migración 100% aditiva**: `mitreTechniques` + `sigmaRulesSynced` + `kevCatalog`; fix de auditoría H1: `CURRENT_SCHEMA_VERSION` realineado 22→24 con el schema declarado). **Sync MITRE real**: descarga el bundle STIX 2.1 oficial de `mitre-attack/attack-stix-data` (~54 MB) con **extractor en streaming** (escáner de profundidad de llaves objeto a objeto — memoria plana, sin JSON.parse del archivo entero; bug de re-escaneo por chunk encontrado y corregido en QA) → 697 técnicas v19.2 almacenadas, fusionadas sobre las 61 curadas (nombres/desc/plataformas/sub-técnicas oficiales ganan; detección ES y relatedTools del bundle se conservan; revocadas/deprecadas excluidas). **Sync Sigma real**: 143 reglas oficiales de `SigmaHQ/sigma` (rules/windows/builtin/security) vía CDN jsDelivr con fallback a la API de GitHub — mismo validador que el import manual; **fix del parser YAML-subset** (las listas por bloque `key:` + `- item` — que TODAS las reglas oficiales usan — rompían el parser original: frames con owner/key, soporte de estilo indentado y misma-indentación + items-mapa anidados; 47/48 reglas de muestra pasan, la restante es un 404 real del CDN y se salta con conteo honesto). **CISA KEV real**: catálogo oficial `cisagov/kev-data` (1.721 CVEs) con *Check for Updates* por hash del CDN; el **CVE Search** muestra tras cada búsqueda el badge **CISA KEV — KNOWN EXPLOITED** (fechas, ransomware, required action — offline tras sync) + **EPSS** de FIRST (cacheado 24 h en tiCache, ~0.99 para Log4Shell verificado). **Backup 3.10.0**: exporta `mitreTechniques.json`/`sigmaRulesSynced.json`/`kevCatalog.json` (verificado en los bytes del ZIP real: 4.97 MB, formatVersion 3.10.0, datos presentes) con import latest-wins por syncedAt/fetchedAt + 3 contadores de conflicto nuevos. E2E navegador: sync KEV ✓ (1721), sync Sigma ✓ (143/144, 1 skip), sync MITRE ✓ (697), MITRE Explorer 223 técnicas padre con técnicas nuevas visibles (T1613/T1659), Sigma Explorer 171 reglas con reglas oficiales abribles (Powerview/DCSync: HIGH/TEST/T1098/condition/YAML), CVE-2021-44228 → KEV badge + EPSS 100%, **offline 6/6 botones deshabilitados** + indicador Offline + todo lo demás funcional, 0 errores de consola/página/dev.log (el DataCloneError del log es el artefacto documentado del stub de prueba del file picker; el fallback funciona por diseño). `eslint` 0 · `tsc --noEmit` 0. Sin UI rota: mismas vistas, mismos flujos, solo la capa nueva.
