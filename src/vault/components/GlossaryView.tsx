...
// Solo el TIPO — el catálogo de 194 términos se carga lazy al abrir el modal
// "Packs" (no pesa el chunk del glosario hasta que se pide).
import type { PackTerm, GlossaryPack } from '../data/iamGlossaryPacks';
...
interface GlossaryViewProps {
  ...
  onCreateTerm: () => void;
  /** PACKS DE GLOSARIO (IAM/GRC/SOC) — importación bulk con dedupe. */
  onImportPackTerms: (incoming: PackTerm[]) => Promise<{ added: number; skipped: number }>;
...
export const GlossaryView: React.FC<GlossaryViewProps> = ({
  ...
  // PACKS DE GLOSARIO (IAM/GRC/SOC) — modal con carga lazy del catálogo.
  const [isPackModalOpen, setIsPackModalOpen] = useState(false);
  const [packs, setPacks] = useState<GlossaryPack[] | null>(null);
  ...
  const [isImportingPacks, setIsImportingPacks] = useState(false);

  const handleOpenPackModal = async () => {
    setIsPackModalOpen(true);
    setPackResult(null);
    if (packs === null && !packsLoading) {
      setPacksLoading(true);
      try {
        const mod = await import('../data/iamGlossaryPacks');
        setPacks(mod.IAM_GLOSSARY_PACKS);
        // Por defecto TODOS seleccionados — un clic y entra el pack completo.
        setSelectedPackIds(new Set(mod.IAM_GLOSSARY_PACKS.map((p) => p.id)));
      } catch (e) {
        console.error('Error cargando packs de glosario:', e);
        setPackResult('No se pudieron cargar los packs.');
      } finally {
  ...
  const handleImportPacks = async () => {
    if (!packs || selectedPackIds.size === 0 || isImportingPacks) return;
    setIsImportingPacks(true);
    setPackResult(null);
    try {
      const incoming: PackTerm[] = packs
        .filter((p) => selectedPackIds.has(p.id))
        .flatMap((p) => p.terms);
      const { added, skipped } = await onImportPackTerms(incoming);
      setPackResult(
        skipped > 0
          ? `${added} términos agregados · ${skipped} omitidos (ya existían en tu glosario).`
          : `${added} términos agregados. ¡A estudiar!`
      );
    } catch (e) {
      console.error('Error importando packs:', e);
      setPackResult('Hubo un error al importar. Inténtalo de nuevo.');
    } finally {
      setIsImportingPacks(false);
    }
  };
    ...
    <div className="flex flex-col flex-1 h-[calc(100vh-48px)] overflow-hidden bg-[#0A0A0A] relative">
      ...
      <div className="px-6 py-3 border-b border-[#262626] bg-[#0D0D0D] flex flex-wrap items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* PACKS DE GLOSARIO — catálogos curados (IAM ~138 · GRC · SOC). */}
          <button
            onClick={() => void handleOpenPackModal()}
            ...
            title="Importar packs curados: IAM (conceptos, protocolos, acceso, plataformas) + GRC + SOC"
          >
      ...
      {/* 4. Modal PACKS DE GLOSARIO — catálogos curados IAM (~138) + GRC + SOC. */}
      {isPackModalOpen && (
        <div
          className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4"
          role="dialog"
          aria-modal="true"
          aria-label="Importar packs de glosario"
          onClick={() => setIsPackModalOpen(false)}
        >
          <div
            className="bg-[#0D0D0D] border border-[#262626] rounded-lg w-full max-w-lg max-h-[85vh] flex flex-col"
          ...
          >
            <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-[#262626]">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-white">Packs de glosario</h3>
                  <p className="text-[10px] text-[#777]">
                    Catálogos curados, enfocados en IAM. Se omiten los términos que ya tengas.
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
              {packsLoading && (
                <div className="flex flex-col items-center gap-2 py-8 text-[#666]">
                  <Package className="w-6 h-6 animate-pulse" />
                  <p className="text-xs">Cargando catálogos…</p>
                </div>
              )}
              {!packsLoading && packs === null && (
              ...
              {!packsLoading && packs !== null &&
                packs.map((p) => {
                  const isChecked = selectedPackIds.has(p.id);
                  return (
                    <label
                      key={p.id}
                      className={`p-3 rounded border cursor-pointer transition-colors flex items-start gap-3 ${
                        isChecked
                          ...
                          : 'border-[#262626] bg-[#161616]/40 hover:border-[#333]'
                      }`}
                    >
                      <input
                        type="checkbox"
                        ...
                        className="accent-blue-500 cursor-pointer mt-0.5 shrink-0"
                        aria-label={`Seleccionar pack ${p.name}`}
                      />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs font-bold text-white truncate">{p.name}</span>
                          <span className="text-[10px] font-mono text-blue-400 shrink-0">
                            {p.terms.length} términos
                          </span>
                        </div>
                        <p className="text-[11px] text-[#777] mt-0.5 leading-relaxed">{p.description}</p>
                      </div>
                    </label>
                  );
                })}
            ...
            <div className="px-4 py-3 border-t border-[#262626] flex flex-col gap-2">
              {packResult && (
                <p className="text-[11px] text-green-400 bg-green-500/10 border border-green-500/30 rounded px-2.5 py-1.5">
                  {packResult}
                </p>
              )}
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] font-mono text-[#555]">
                  {packs
                    ? `${packs.filter((p) => selectedPackIds.has(p.id)).reduce((acc, p) => acc + p.terms.length, 0)} términos seleccionados`
                    : '—'}
                </span>
                <button
                  onClick={() => void handleImportPacks()}
                  disabled={packs === null || selectedPackIds.size === 0 || isImportingPacks}
                  className="flex items-center gap-1.5 px-4 py-2 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Package className="w-3.5 h-3.5" />
                  {isImportingPacks ? 'Importando…' : 'Importar seleccionados'}
                </button>
              </div>
            </div>
          </div>
